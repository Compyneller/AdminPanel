...
const Card2 = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    homepage?.data?.about?.content_boxes.splice(0, 1, {
      heading: heading,
      text: text,
    });
    const body = {
      ...
      <Card.Body>
        <form action="" onSubmit={(e) => handleSubmit(e)}>
          <InputGroup className="mb-3">
            <Form.Control
              placeholder={homepage?.data?.about?.content_boxes[0]?.heading}
              aria-label="Username"
          ...
          <InputGroup className="mb-3">
            <Form.Control
              placeholder={homepage?.data?.about?.content_boxes[0]?.text}
              aria-label="Username"
              aria-describedby="basic-addon1"
              value={text}
